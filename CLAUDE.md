...
- Tests en iPhone 17 Pro / iOS 27.0: 65 tests, 14 suites, correctos tras añadir precios, tiendas web, recuperación de `recordChangeTag`, filtro por categoría, regresiones de Amazon/PlayStation/IKEA/Nintendo y captura web renderizada genérica.
...
- Amazon también intenta la captura de metadatos y del DOM renderizado una vez al añadir, con fallback al ASIN. El precio visible semántico (`priceToPay`/equivalentes genéricos) prevalece sobre precios secundarios o metadatos ambiguos. Continúa sin scraping periódico y mantiene el enlace a Keepa.
- El renderizador usa ocho segundos como límite blando: algunas tiendas mantienen recursos abiertos y nunca llaman a `didFinish`, aunque el producto ya esté en el DOM. Al vencer el plazo detiene nuevas cargas y captura lo ya renderizado en vez de descartar la página completa.
...
- Captura DOM real desde Safari en dispositivo físico y alta del enlace concreto de Nintendo una vez superada su cola; no se recibió una URL exacta de AliExpress ni de los casos de Amazon restantes para validar sus posibles variantes, cupones o bloqueos.
...
- El enlace concreto de Nintendo no falla por un selector: una petición normal devuelve `307` hacia `nintendostoreuk.queue-it.net`. Cuando se obtiene la página real, sus 1,3 MB de HTML contienen el producto en `props.pageProps.product` dentro de `__NEXT_DATA__`; el parser comprobado sobre esa respuesta extrae título, `59,99 EUR`, imagen y categoría. No se falsea el user-agent como Googlebot para eludir Queue-it.
